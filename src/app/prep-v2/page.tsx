...
export default function NewPreparationPage() {
  ...
  const addLog = (msg: string) => {
    setPipelineLogs(prev => [...prev.slice(-4), `> ${msg}`]);
  };
        ...
        <div className="space-y-12">
          ...
          <div className="text-center space-y-4">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-blue-50 text-blue-600 rounded-full text-[10px] font-black uppercase tracking-widest border border-blue-100/50">
              <SecurityShield size={12} strokeWidth={3} className="animate-pulse" />
          ...
          {(isProcessing || manifest) && (
            <div className="space-y-10">
                ...
                <AnimatePresence mode="popLayout">
                  {pipelineLogs.map((log, i) => (
                    <motion.p 
                      ...
                      className={cn(
                        "tracking-tight",
                        i === pipelineLogs.length - 1 ? "text-blue-400 font-bold" : "text-white/40"
                      )}
                    >
                      {log}
                    </motion.p>
              ...
              <div className="relative h-2 w-full bg-slate-100 rounded-full overflow-hidden shadow-inner">
                <motion.div 
                  initial={{ width: "0%" }}
                  animate={{ width: `${((activeState + 1) / PIPELINE_STATES.length) * 100}%` }}
