...
import { Folder, Wallet, Menu, User, LogOut, Settings, LayoutDashboard, X, Star } from "lucide-react";
...
export function Shell({ children, activeTab, setActiveTab, onDashboardClick, user, profile }: ShellProps) {
  ...
  return (
    <div className="flex min-h-screen w-full bg-[#FAFAFA] text-[#0F172A] selection:bg-blue-100 font-sans overflow-x-hidden">
      ...
      <header className="fixed top-0 left-0 right-0 h-16 bg-white/80 backdrop-blur-xl border-b border-black/[0.03] z-[60] px-4 flex items-center justify-between">
        ...
        <div className="relative z-[62]">
          ...
          <AnimatePresence>
            {profileOpen && user && (
              <>
                <div className="fixed inset-0 z-[61]" onClick={() => setProfileOpen(false)} />
                <motion.div
                  initial={{ opacity: 0, scale: 0.95, y: 10 }}
                  animate={{ opacity: 1, scale: 1, y: 0 }}
                  exit={{ opacity: 0, scale: 0.95, y: 10 }}
                  className="absolute right-0 mt-2 w-48 bg-white rounded-2xl shadow-xl border border-black/[0.05] z-[62] py-2 overflow-hidden"
                >
                  <button 
                    onClick={() => {
                      onDashboardClick?.();
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSidebarOpen(false)} className="fixed inset-0 bg-black/20 backdrop-blur-sm z-[70]" />
            <motion.aside
              initial={{ x: "-100%" }}
              animate={{ x: 0 }}
              exit={{ x: "-100%" }}
            ...
            >
              <div className="flex items-center justify-between mb-8">
                ...
                <button onClick={() => setSidebarOpen(false)} className="p-2 hover:bg-black/[0.03] rounded-full"><X size={20} /></button>
              </div>
              ...
              <div className="space-y-8 overflow-y-auto">
                <div>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center text-sm font-bold"><span className="text-[#64748B]">Photos</span><span>{profile?.photo_count ?? 0}</span></div>
                    <div className="flex justify-between items-center text-sm font-bold"><span className="text-[#64748B]">Documents</span><span>{profile?.doc_count ?? 0}</span></div>
                    <div className="flex justify-between items-center text-sm font-bold"><span className="text-[#64748B]">Videos</span><span>{profile?.video_count ?? 0}</span></div>
                    <div className="flex justify-between items-center text-sm font-bold"><span className="text-[#64748B]">Starred</span><span className="text-amber-500">{profile?.starred_count ?? 0}</span></div>
                  </div>
                </div>
                ...
                <div className="pt-6 border-t border-black/[0.05]">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="text-[11px] font-bold text-[#64748B]">{storageUsedGB} GB of {storageLimitGB} GB used</div>
                  </div>
                </div>

                {profile?.role && profile.role !== 'USER' && (
                  <div className="pt-6 border-t border-black/[0.05]">
                    <div className="px-3 py-2 bg-blue-50 text-blue-600 rounded-xl text-[10px] font-black uppercase tracking-widest text-center">{profile.role} ACCESS</div>
                  </div>
                )}
              </div>
            </motion.aside>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 flex h-20 items-start justify-around bg-white/80 backdrop-blur-3xl border-t border-black/[0.03] px-6 pt-3">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={cn("flex flex-col items-center gap-1.5 transition-all px-6 py-2 rounded-2xl", activeTab === tab.id ? "text-[#3B82F6]" : "text-[#94A3B8]")}
          >
            <tab.icon size={24} strokeWidth={activeTab === tab.id ? 2.5 : 2} className={cn("transition-transform", activeTab === tab.id && "scale-110")} />
